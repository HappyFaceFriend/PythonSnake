...
class SnakeAI:
    ...
    def pre_movement(self):  # called at each frame just before movement of snake
        ...
        self.collision = False
        bodies = self.get_snake_bodies_until(len(self.snake.body)//3)
        for body in bodies:
            if body[0][0] == self.apple_pos[0] and body[0][1] == self.apple_pos[1]: # can eat apple before collision
                    break                                                           
            for i in range(4, len(body)):
                if body[0][0] == body[i][0] and body[0][1] == body[i][1]: 
                    self.collision = True
                    self.command_list.clear()
                    dir = Vector2(body[i][0] - body[i-1][0], body[i][1] - body[i-1][1]) 

                    self.command_list.append(dir)
                    break
